...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --source)
            ...
            ;;
        --help|-h)
            echo "Usage: $0 [--dry-run] [--scope <scope>] [--source agents|claude]"
            echo ""
            echo "Options:"
            echo "  --dry-run        Show what would change without modifying files"
            echo "  --scope          Only sync specific scope (root)"
            echo "  --source         Source of truth file: agents or claude (default: agents)"
            exit 0
            ;;
        *)
...
# Map scope to source of truth file path
get_source_path() {
    local scope="$1"
    local dir=""

    case "$scope" in
        root) echo "$REPO_ROOT/$SOURCE_FILE"; return ;;
        sdk)  dir="$REPO_ROOT/prowler" ;;
        *)    dir="$REPO_ROOT/$scope" ;;
    esac

    if [ -d "$dir" ] && [ -f "$dir/$SOURCE_FILE" ]; then
        echo "$dir/$SOURCE_FILE"
    else
        echo ""
    fi
}
...
echo -e "${BLUE}Skill Sync - Updating $SOURCE_FILE Auto-invoke sections${NC}"
echo "========================================================"
echo ""

# Collect skills by scope using temp directory (bash 3 compatible, no associative arrays)
SCOPE_DIR=$(mktemp -d)
...
while IFS= read -r skill_file; do
    ...
    # Protect pipe by replacing with ;; (restored later)
    ...
    for scope in "${scopes[@]}"; do
        ...
        # Append to scope file (one entry per line)
        echo "$skill_name:$auto_invoke" >> "$SCOPE_DIR/$scope"
    done
...
for scope_file in $(find "$SCOPE_DIR" -type f -print | sort); do
    scope=$(basename "$scope_file")
    agents_path=$(get_source_path "$scope")

    if [ -z "$agents_path" ] || [ ! -f "$agents_path" ]; then
        echo -e "${YELLOW}Warning: No $SOURCE_FILE found for scope '$scope'${NC}"
        continue
    fi

    echo -e "${BLUE}Processing: $scope -> $(basename "$(dirname "$agents_path")")/$SOURCE_FILE${NC}"
...
| $action | \`$skill_name\` |"
    ...
    if $DRY_RUN; then
        ...
        if grep -q "### Auto-invoke Skills" "$agents_path"; then
            ...
            # Insert after Available Skills section (supports both blockquote and table formats)
            awk '
                /SKILL\.md\)/ { last_skill_line = NR }
                NR == last_skill_line + 1 && !inserted {
                    if (/^$/ || /^---$/) {
                        print ""
                        while ((getline line < "'"$section_file"'") > 0) print line
                        close("'"$section_file"'")
